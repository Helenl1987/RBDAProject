...
bfile = open('/Users/zimoli/Downloads/RBDA-MCINTOSH/Project/RBDAProject/spark_data/business_lv_30000.txt')
...
df = sqlContext.createDataFrame(rdd, ['name', 'neighborhood','business_id','main_cate','cate_count','longitude','latitude','cuisines','cuisine_count','dates'])
df_d = df.na.drop()

df_d_f = df_d.filter((df_d.longitude != 'NULL') & (df_d.latitude != 'NULL'))

all_list = sc.broadcast([[str(row['business_id']),str(row['name']),str(row['neighborhood']),str(row['main_cate']),str(row['latitude']),str(row['longitude']),str(row['dates']),str(row['cuisines']),str(row['cate_count']),str(row['cuisine_count'])] for row in df_d_f.collect()])
...
pt_cate_ts.saveAsPickleFile('/Users/zimoli/Downloads/RBDA-MCINTOSH/Project/RBDAProject/spark_data/geoprocess_lv_output_30000')
